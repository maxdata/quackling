...
def test_chunk_without_metadata():
    with open("data/0_inp_dl_doc.json") as f:
        data_json = f.read()
    ...
    with open("data/0_out_chunks_wout_meta.json") as f:
...
def test_chunk_with_metadata():
    ...
    chunks = chunker.chunk(dl_doc=dl_doc)
    act_data = dict(root=[n.model_dump() for n in chunks])
    with open("data/0_out_chunks_with_meta.json") as f:
        exp_data = json.load(fp=f)
    assert exp_data == act_data

if __name__ == "__main__":
    test_chunk_without_metadata()
